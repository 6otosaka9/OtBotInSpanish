...
• Limit: ${limitawal-updated.limit}`
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`𝘆𝗮𝗵𝗵 @${mentioned[0].split('@')[0]} ya no eres admin🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Felicidades🥳 @${mentioned[0].split('@')[0]} *te conviertes en administrador * (+ _ +)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ activar bienvenida/izquierda en este grupo ')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭  deshabilitar bienvenida/izquierda en este grupo ')
					} else {
						reply(ind.satukos())
					}
					break 
					...
					if (Number(args[0]) === 1) {
						...
						samih.splice(from, 1)
						fs.writeFileSync('./database/bot/simi.json', JSON.stringify(samih))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ APAGANDO SIMI EN ESTE GRUPO')
					} else {
						reply(ind.satukos())
					}
					break
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ APAGANDO NSFW EN ESTE GRUPO')
					} else {
						reply(ind.satukos())
					}
				break
